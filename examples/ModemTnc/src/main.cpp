/*
AFSK modem firmware with selectable KISS transport(s):
- TNC_TRANSPORT_BLE=1 enables BLE-KISS API over NimBLE
- TNC_TRANSPORT_SERIAL=1 enables USB serial KISS

RF/audio path uses esp32-afsk + arduino-audio-tools + DRA818.
*/
...
#ifndef TNC_TRANSPORT_BLE
#define TNC_TRANSPORT_BLE 1
#endif

#ifndef TNC_TRANSPORT_SERIAL
#define TNC_TRANSPORT_SERIAL 0
#endif

#if !TNC_TRANSPORT_BLE && !TNC_TRANSPORT_SERIAL
...
#if TNC_TRANSPORT_BLE
using BleKiss = BleKissTnc<512, 384, 384, 3>;

static BleKiss::Config makeBleConfig() {
  BleKiss::Config cfg;
  cfg.deviceName = "ESP32 AFSK BLE KISS";
  cfg.preferredMtu = 185;
  cfg.requireNotifySubscription = true;
  cfg.maxNotifyChunksPerLoop = 1;
  return cfg;
}
...
static void on_tx_samples(const float *samples, size_t count) {
  ...
  if (count > TX_BUFFER_SAMPLES) {
    count = TX_BUFFER_SAMPLES;
  }
...
static void switch_to_rx_audio() {
  ...
  if (!adc_active) {
    ...
    config.sample_rate = AUDIO_SAMPLE_RATE_HZ * 1.02f;
...
#if TNC_TRANSPORT_BLE
static void on_ble_kiss_frame(const uint8_t *data, size_t len, void *ctx) {
  (void)ctx;
  ...
  const uint8_t command = data[0] & 0x0F;
...
#if TNC_TRANSPORT_SERIAL
static void kiss_write_escaped_serial(uint8_t b) {
  if (b == KISS_FEND) {
...
static void kiss_send_frame_serial(const uint8_t *data, size_t len) {
  Serial.write(KISS_FEND);
  Serial.write(KISS_CMD_DATA);
  for (size_t i = 0; i < len; ++i) {
    kiss_write_escaped_serial(data[i]);
  }
  Serial.write(KISS_FEND);
}

static void handle_kiss_rx_serial() {
  while (Serial.available() > 0) {
    ...
    if (b == KISS_FEND) {
      if (kiss_in_frame && kiss_payload_len > 0 && kiss_port_cmd == KISS_CMD_DATA) {
        begin_tx();
        mod.modulate(kiss_payload_buf, kiss_payload_len, tx_mod_buffer,
                     TX_BUFFER_SAMPLES, TX_LEAD_SILENCE_MS, TX_TAIL_SILENCE_MS);
        end_tx();
      }
...
#if TNC_TRANSPORT_BLE
  if (!bleKiss.sendDataFrame(frame, len, 0)) {
    Serial.println("BLE KISS TX drop (queue full / not ready)");
  }
#endif

#if TNC_TRANSPORT_SERIAL
  kiss_send_frame_serial(frame, len);
#endif
}
...
static void init_radio_module() {
  ...
  const bool grp_ok = radio.group(DEFAULT_RF_BANDWIDTH,
                                  DEFAULT_RF_FREQ_TX,
                                  DEFAULT_RF_FREQ_RX,
                                  DEFAULT_RF_CTCSS_TX,
                                  DEFAULT_RF_SQUELCH,
                                  DEFAULT_RF_CTCSS_RX);
  const bool vol_ok = radio.volume(DEFAULT_VOLUME);
  const bool fil_ok = radio.filters(DEFAULT_RF_PREEMPH,
                                    DEFAULT_RF_HIGHPASS,
                                    DEFAULT_RF_LOWPASS);
...
#if TNC_TRANSPORT_BLE
  bleKiss.setFrameCallback(on_ble_kiss_frame);
  bleKiss.setConnectCallback(on_ble_connect);
  bleKiss.setDisconnectCallback(on_ble_disconnect);
...
#if TNC_TRANSPORT_BLE && TNC_TRANSPORT_SERIAL
  Serial.println("Mode: BLE + USB serial KISS transport");
#elif TNC_TRANSPORT_BLE
  Serial.println("Mode: BLE KISS transport");
#else
  Serial.println("Mode: USB serial KISS transport");
#endif

  Serial.println("AFSK modem ready");
}
...
#if TNC_TRANSPORT_SERIAL
  handle_kiss_rx_serial();
#endif

  if (adc_active) {
    const size_t bytes = adc.readBytes((uint8_t *)rx_pcm_i16, sizeof(rx_pcm_i16));
    if (bytes > 0) {
